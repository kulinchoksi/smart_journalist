...
class SmartJournalistCrew():
    ...
    def __init__(self):
        ...
        self.reasoning_llm = LLM(
            model=os.getenv("REASONING_MODEL_NAME", "vertex_ai/gemini-2.5-pro"),
            temperature=float(os.getenv("TEMPERATURE", 0.0)),
            project=os.getenv("GOOGLE_CLOUD_PROJECT"),
            location=os.getenv("GOOGLE_CLOUD_LOCATION", "us-central1"),
            vertex_credentials=vertex_credentials_json,
    ...
    @agent
    def political_news_specialist(self) -> Agent:
        """Creates the Political News Specialist agent"""
        return Agent(
            config=self.agents_config['political_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def political_news_search(self) -> Task:
        """Creates the political news search task"""
        return Task(
            config=self.tasks_config['political_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def political_news_content(self) -> Task:
        """Creates the political news content task"""
        return Task(
            config=self.tasks_config['political_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def political_news_verification(self) -> Task:
        """Creates the political news search task"""
        return Task(
            config=self.tasks_config['political_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # economy news agent and tasks
    @agent
    def economy_news_specialist(self) -> Agent:
        """Creates the Economy News Specialist agent"""
        return Agent(
            config=self.agents_config['economy_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def economy_news_search(self) -> Task:
        """Creates the economy news search task"""
        return Task(
            config=self.tasks_config['economy_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def economy_news_content(self) -> Task:
        """Creates the economy news content task"""
        return Task(
            config=self.tasks_config['economy_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def economy_news_verification(self) -> Task:
        """Creates the political news search task"""
        return Task(
            config=self.tasks_config['economy_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # Technology news agent and tasks
    @agent
    def technology_news_specialist(self) -> Agent:
        """Creates the Technology News Specialist agent"""
        return Agent(
            config=self.agents_config['technology_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def technology_news_search(self) -> Task:
        """Creates the technology news search task"""
        return Task(
            config=self.tasks_config['technology_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def technology_news_content(self) -> Task:
        """Creates the technology news content task"""
        return Task(
            config=self.tasks_config['technology_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def technology_news_verification(self) -> Task:
        """Creates the technology news search task"""
        return Task(
            config=self.tasks_config['technology_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # Science news agent and tasks
    @agent
    def science_news_specialist(self) -> Agent:
        """Creates the Science News Specialist agent"""
        return Agent(
            config=self.agents_config['science_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def science_news_search(self) -> Task:
        """Creates the science news search task"""
        return Task(
            config=self.tasks_config['science_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def science_news_content(self) -> Task:
        """Creates the science news content task"""
        return Task(
            config=self.tasks_config['science_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def science_news_verification(self) -> Task:
        """Creates the science news search task"""
        return Task(
            config=self.tasks_config['science_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # Stock market news agent and tasks
    @agent
    def stock_market_news_specialist(self) -> Agent:
        """Creates the stock market News Specialist agent"""
        return Agent(
            config=self.agents_config['stock_market_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def stock_market_news_search(self) -> Task:
        """Creates the stock market news search task"""
        return Task(
            config=self.tasks_config['stock_market_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def stock_market_news_content(self) -> Task:
        """Creates the stock market news content task"""
        return Task(
            config=self.tasks_config['stock_market_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def stock_market_news_verification(self) -> Task:
        """Creates the stock market news search task"""
        return Task(
            config=self.tasks_config['stock_market_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # Energy news agent and tasks
    @agent
    def energy_news_specialist(self) -> Agent:
        """Creates the energy News Specialist agent"""
        return Agent(
            config=self.agents_config['energy_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def energy_news_search(self) -> Task:
        """Creates the Energy news search task"""
        return Task(
            config=self.tasks_config['energy_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def energy_news_content(self) -> Task:
        """Creates the Energy news content task"""
        return Task(
            config=self.tasks_config['energy_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def energy_news_verification(self) -> Task:
        """Creates the Energy news search task"""
        return Task(
            config=self.tasks_config['energy_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )
    
    # Infrastructure news agent and tasks
    @agent
    def infrastructure_news_specialist(self) -> Agent:
        """Creates the Infrastructure News Specialist agent"""
        return Agent(
            config=self.agents_config['infrastructure_news_specialist'],
            tools=[],
            llm=self.llm,
        )

    @task
    def infrastructure_news_search(self) -> Task:
        """Creates the Infrastructure news search task"""
        return Task(
            config=self.tasks_config['infrastructure_news_search'],
            tools=[self.news_search_tool],
        )
    
    @task
    def infrastructure_news_content(self) -> Task:
        """Creates the Infrastructure news content task"""
        return Task(
            config=self.tasks_config['infrastructure_news_content'],
            tools=[self.web_scraper_tool],
        )
    
    @task
    def infrastructure_news_verification(self) -> Task:
        """Creates the Infrastructure news search task"""
        return Task(
            config=self.tasks_config['infrastructure_news_verification'],
            tools=[self.news_search_tool, self.web_scraper_tool],
            llm=self.reasoning_llm,
        )

    # final reporting agent and task
    @agent
    def news_reporting_specialist(self) -> Agent:
        """Creates the News Reporting Specialist agent"""
        return Agent(
            config=self.agents_config['news_reporting_specialist'],
            tools=[],
            llm=self.llm,
        )
    ...
    def crew(self) -> Crew:
        ...
        return Crew(
            ...
            verbose=True,
            output_log_file=f"logs/logs_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
            # memory=True,
            # planning=True,
            # embedder={
